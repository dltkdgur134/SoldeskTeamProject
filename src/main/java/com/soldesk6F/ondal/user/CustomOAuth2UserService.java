...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		User user = userRepository.findById(email).map(entity -> entity.update(name))
				.orElseGet(() -> userRepository.save(User.builder().userId(email) // 소셜 이메일을 아이디로 쓴다고 가정
						...
						.userProfileName(profilePhotoName).userProfileExtension(extension).nickName(nickName)
						.userPhone(tel).build()));
